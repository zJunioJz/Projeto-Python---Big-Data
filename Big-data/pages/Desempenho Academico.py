...
if uploaded_file is not None:
    ...
    if len(colunas_presentes) != len(colunas_necessarias):
        ...
        tabela['Turma'] = tabela['Turma'].fillna('Desconhecido').astype(str).str.strip()

        # Obter todas as turmas únicas
        turmas = sorted(set(tabela['Turma'].unique()), key=str.lower)
        
        # Verifica se a turma '1604' está na lista de turmas
        if '1604' not in turmas:
            st.warning("A turma '1604' não está presente nos dados.")
        else:
            # Seleciona a turma
