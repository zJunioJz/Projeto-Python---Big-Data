...
if uploaded_file is not None:
    ...
    if colunas_faltantes:
        ...
        colunas_desejadas = ['Nome', 'Turma', 'Idade -Cálculo média'] + colunas_selecionadas
        colunas_existem = [coluna for coluna in colunas_desejadas if coluna in aluno_data.columns]

        if colunas_existem:
            st.write(f"### Dados Cadastrais do Aluno: {selected_aluno} - Turma: {aluno_data['Turma'].values[0]}")
            st.dataframe(aluno_data[colunas_existem])
        else:
            st.error("Nenhuma das colunas especificadas está presente no DataFrame.")
