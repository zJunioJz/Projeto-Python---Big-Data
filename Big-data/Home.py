...
if colunas_grafico_fisio:
    ...
    # Substituir valores infinitos por NaN antes de calcular a média
    tabela_fisio.replace([float('inf'), float('-inf')], pd.NA, inplace=True)

    # Calcular a média da turma para as colunas selecionadas e arredondar para 2 casas decimais
    media_turma = tabela_fisio[tabela_fisio['Turma'] == selected_turma][colunas_grafico_fisio].mean().round(2)

    # Gráfico comparando o aluno com a média da turma
    ...
    fig_comparacao.update_layout(
        ...
        yaxis=dict(
            tickfont=dict(
                size=15  # Tamanho da fonte para o eixo y
            )
        )
    )
